...
public class AvaliacaoSerieService {
    ...
    public AvaliacaoSerieDTOResponse criar(SerieDTORequest dto) {
        Usuario usuario = usuarioRepository.findById(dto.getUsuarioId())
                .orElseThrow(() -> new RecurseNotFoundException("Usuário", dto.getUsuarioId()));
 
        Serie serie = serieRepository.findById(dto.getSerieId())
                .orElseThrow(() -> new RecurseNotFoundException("Série", dto.getSerieId()));
 
        AvaliacaoSerie avaliacao = new AvaliacaoSerie();
        avaliacao.setNota(dto.getNotaMedia());
    ...
    public AvaliacaoSerieDTOResponse buscarPorId(UUID id) {
        AvaliacaoSerie avaliacao = avaliacaoSerieRepository.findById(id)
                .orElseThrow(() -> new RecurseNotFoundException("Avaliação de série", id));
