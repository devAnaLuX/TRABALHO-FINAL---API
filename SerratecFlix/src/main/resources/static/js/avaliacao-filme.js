const BASE_URL = 'http://localhost:8080/avaliacoes_filme';
...
function renderCard(a, container) {
    ...
    card.innerHTML = `
        ...
        <div class="card-body">
            <div class="title">Avaliação de Filme</div>
            <div class="meta">
                <span class="tag">📅 ${data}</span>
            </div>
            <div class="desc">${a.comentario || 'Sem comentário.'}</div>
            <div class="detail-row"><span class="detail-label">ID Usuário</span><span class="detail-value">${a.usuarioId}</span></div>
            <div class="detail-row"><span class="detail-label">ID Filme</span><span class="detail-value">${a.filmeId}</span></div>
            <div class="actions">
...
async function cadastrar() {
    ...
    if (!filmeId)    { toast('ID do filme é obrigatório.', 'error'); return; }

    const body = { nota: parseFloat(nota), comentario, usuarioId, filmeId };
...
function abrirEdicao(a) {
    ...
    document.getElementById('e-filme').value      = a.filmeId    || '';
}
...
function limparForm(p) {
    [`${p}-nota`, `${p}-comentario`, `${p}-usuario`, `${p}-filme`]
